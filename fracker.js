...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function main() {
        ...
        centers[centers.length - 1].color = bigcolor
        centers[centers.length - 1].radius = dotsize
        // canvas_context.drawImage(canvas, -2 * Math.cos(counter), -2 * Math.sin(counter))
        // canvas_context.drawImage(canvas, Math.random() - .5, Math.random() - .5)
        ...
        // runner.x = 350 + Math.cos(counter * .01) * 1
        // runner.y = 350 + Math.sin(counter * .01) * 1
        centers[0].x = 350
        centers[0].y = 350

        let dist = sizevalue.value
        for (let t = 1; t < counters.length; t++) {
            counters[t] += (Math.pow(-(parameter.value), t)) / (Math.PI * 2 * (speedvalue.value))
            centers[t].x = centers[t - 1].x + Math.cos(counters[t]) * dist
            centers[t].y = centers[t - 1].y + Math.sin(counters[t]) * dist
            dist *= .33333
        }


        centers[centers.length - 1].draw()
        // runner.x += Math.cos(zcounter)*50
        // runner.y += Math.sin(zcounter)*50

        // runner.x += Math.sin(zcounter * counter) * 30
        // runner.y += Math.cos(zcounter * counter) * 30
